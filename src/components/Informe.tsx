...
export default function Informe({ registros }: Props) {
  ...
  // Filtrar registros por rango de fechas y cliente
      ...
      <div className="bg-[var(--c-card)] border border-[var(--c-border)] rounded-2xl p-4 sm:p-6">
        ...
        <div className="flex gap-2 mb-5">
          {(['todos', 'Carbon Free', 'Matrix'] as const).map((c) => (
            <button
              key={c}
              onClick={() => { setClienteFiltro(c); setInforme(''); setError(''); }}
              className={`px-4 py-1.5 rounded-lg text-xs font-mono border transition-all ${
                clienteFiltro === c
                  ? 'bg-amber-500/20 border-amber-500/50 text-amber-300'
                  : 'bg-transparent border-[var(--c-border)] text-slate-500 hover:text-[var(--c-text-2)] hover:border-slate-500'
              }`}
            >
              {c === 'todos' ? 'Todos' : c}
            </button>
        ...
        <div className="bg-[var(--c-card)] border border-[var(--c-border)] rounded-2xl overflow-hidden">
          ...
          <div className="px-6 py-3 border-t border-[var(--c-border-sub)] flex items-center justify-between">
            <p className="font-mono text-xs text-slate-600">
              Generado con IA · {clienteFiltro !== 'todos' ? `${clienteFiltro} · ` : ''}{registrosFiltrados.length} eventos · {formatDate(fechaDesde)} — {formatDate(fechaHasta)}
            </p>
            <button
              onClick={generarInforme}
              disabled={generando}
